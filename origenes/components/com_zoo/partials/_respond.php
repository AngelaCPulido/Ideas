...
<div id="respond">
	...
	<form method="post" action="<?php echo JRoute::_('index.php?option=com_zoo&controller=comment&task=save'); ?>">
	...
	<?php elseif ($active_author instanceof CommentAuthorFacebook) : ?>
		<p class="user">
			...
			- <a class="fb-logout" href="#"><?php echo JText::_('Logout'); ?></a>
		</p>
	<?php elseif ($active_author instanceof CommentAuthorTwitter) : ?>
		<p class="user">
			<?php echo JText::_('Logged in as').' '.$active_author->name.' ('.JText::_('Twitter').')'; ?>
			- <a class="twitter-logout" href="<?php echo JRoute::_('index.php?option=com_zoo&controller=comment&task=twitterlogout&item_id='.$item->id, false); ?>"><?php echo JText::_('Logout'); ?></a>	
		</p>
		...
		<?php if ($params->get('facebook_enable') || $params->get('twitter_enable')) : ?>	
			<p class="connects">
				...
				<?php if ($params->get('facebook_enable')) : ?>
					<fb:login-button onlogin="comments_app.onFacebookLogin();"></fb:login-button>
				<?php endif; ?>
				
				<?php if ($params->get('twitter_enable')) : ?>
				<a class="twitter-connect" href="<?php echo JRoute::_($this->link_base.'&controller=comment&item_id='.$item->id.'&task=twitterconnect', false); ?>">
					<img src="<?php echo JURI::root().'media/zoo/assets/images/connect_twitter.png'; ?>"></img>
				</a>
				<?php endif; ?>
	...
	<?php endif; ?>
		...
		<?php if (!$registered || ($registered && !$active_author->isGuest())) : ?>
			...
			<div class="content">
				<textarea name="content"><?php echo $params->get('content'); ?></textarea>
			</div>
			...
			<input type="hidden" name="redirect" value="<?php echo str_replace('&', '&amp;', YRequest::getString('REQUEST_URI', '', 'server')); ?>"/>
