...
if ($item_count = count($items)) {
	...
	if (!$center = GooglemapsHelper::locate($google_api_key, $location, $cache)) { 
	...
	foreach ($items as $item) {
		...
		foreach ($elements as $element) {
			if (($element->getElementType() == 'googlemaps') && $element->hasValue()) {
				if ($coordinates = GooglemapsHelper::locate($google_api_key, $element->getElementData()->get('location'), $cache)) {
